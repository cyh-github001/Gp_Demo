...
import com.ETL.Tags.{TagAPP, TagDevice, TagKeyWord, TagLocation, TagsAd}
...
/**
  * 上下文标签
  */
object TagsContext03 {
    def main(args: Array[String]): Unit = {
      val days: String = "20190826"
...
//    val Array(inputPath,outputPath,dirPath,stopPath,days)=args
    ...
    // 读取数据
    val df = sQLContext.read.parquet("e:/Test001")
    // 读取字段文件
    val map = sc.textFile("e:/2016-10-01_06_p1_invalid.1475274123982.log").map(_.split("\t",-1))
      .filter(_.length>=5).map(arr=>(arr(4),arr(1))).collectAsMap()
    // 将处理好的数据广播
    val broadcast = sc.broadcast(map)

    // 获取停用词库
    val stopword = sc.textFile("e:/stopwords.txt").map((_,0)).collectAsMap()
    ...
    df.filter(TagUtils.OneUserId)
      ...
      case(userid,userTag)=>{
        ...
        //处理下标签
       val tsgs =  userTag.map(t=>t._1+","+t._2).mkString(",")
